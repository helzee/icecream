...
public class Gui {
    ...
    public static void build() throws SQLException {
        ...
        scrollPanelLeft.setHorizontalScrollBarPolicy(
                JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
        scrollPanelLeft.setVerticalScrollBarPolicy(
        ...
        scrollProducts.setHorizontalScrollBarPolicy(
                JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
        scrollProducts.setVerticalScrollBarPolicy(
                JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
        ...
        scrollModAdds.setHorizontalScrollBarPolicy(
                JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
        scrollModAdds.setVerticalScrollBarPolicy(
                JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
        ...
        scrollModRemoves.setHorizontalScrollBarPolicy(
                JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
        scrollModRemoves.setVerticalScrollBarPolicy(
                JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
        ...
        JLabel orderTotalLabel = new JLabel("PRE-TAX TOTAL:",
    ...
    private static void getItemButtons() { // change so that when clicked it adds the id as well as the name and price to
        ...
        String[] idsproj = Format.rsToArray(Execute.runQuery(
        ...
        String[] idsmod = Format.rsToArray(Execute.runQuery(
                "SELECT id FROM MenuModification WHERE isOffered is TRUE;"));

        for (String id : idsmod) {

            String name = Format.rsToString(Execute
                    .runQuery("SELECT name FROM MenuModification WHERE id = "
                            + id + ";"));
            String price = Format.rsToString(Execute.runQuery(
    ...
    public static void finishOrder() throws SQLException {
        ...
        // start at ilp 1 because 0th index is the title bar (id, name, price) not
        // including the placeholder at the end
        for (int i = 1; i < oi.length; i++) { // go through each item in order and make its respective insert
            ...
            String id = (String) ((JLabel) ((JPanel) ic[0]).getComponent(1))
                    .getClientProperty("id"); // should be itemboxleft
            String name = (((JLabel) ic[1])).getText(); // should be itemboxleft
    ...
    private static void showReciept(String s) {
        ...
        reciept.setText("<html><pre>" + s.replaceAll("<", "&lt;")
    ...
    private static void showEmployeeSelect() throws SQLException {
        ...
        ResultSet employees = Execute.runQuery(
                "SELECT id, firstName, LastName FROM Employee WHERE isCurrentlyEmployed = true;");
        ...
        for (int i = 0; i < empButtons.size(); i++) {
            empButtons.get(i).addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    ...
                    WorkingEmployee.setText("Current Worker: "
                            + ((JButton) e.getSource()).getText());
                    WorkingEmployee.putClientProperty("id",
                            ((int) ((JButton) e.getSource())
                                    .getClientProperty("id")));
                    ((JFrame) (scrollpanel.getParent().getParent().getParent()
                            .getParent())).dispose();
                    try {
                        updateWorkerText();
                    } catch (SQLException e1) {
    ...
    private static void updateRemoveButtons() throws SQLException {
        ...
        String projID = (String) ((JLabel) ((JPanel) ic[0]).getComponent(1))
                .getClientProperty("id"); // should be itemboxleft
        String[] idsitem = Format.rsToArray(Execute.runQuery(
                "SELECT itemId FROM ProductIngredient WHERE productID = "
                        + projID + ";"));

        for (String id : idsitem) {

            String name = Format.rsToString(Execute
                    .runQuery("SELECT name FROM Item WHERE id = " + id + ";"));
    ...
    private static void showPastTransactions() throws SQLException {
        ...
        ResultSet transactionRS = Execute.runQuery(
                "SELECT id, transactionNumber, timeCompleted FROM Transaction ORDER BY timeCompleted DESC LIMIT "
                        + transactionResultLimit + ";");

        while (transactionRS.next()) {
            JPanel tpanel = new JPanel(new BorderLayout());
            tpanel.setMaximumSize(new Dimension(10000, 40));
            tpanel.setSize(new Dimension(10000, 40));
            JLabel tlabel = new JLabel("  " + transactionRS.getString(2) + "  "
                    + transactionRS.getString(3) + "    ");
    ...
    private static boolean TransactionIsRefunded(String txNum)
            throws SQLException {
        ...
        ResultSet transIsRefunded = Execute
                .runQuery("SELECT COALESCE(isRefunded,'t'::boolean) "
                        + "FROM Transaction "
                        + "JOIN TransactionProduct on (TransactionProduct.transactionID = Transaction.ID) "
                        + "WHERE Transaction.transactionNumber = '" + txNum
    ...
    private static void RefundTransactionProducts(String txNum)
            throws SQLException {
        ResultSet txProducts = Execute.runQuery("SELECT TP.id "
                + "FROM Transaction T "
        ...
        while (txProducts.next()) {
            ...
            Update.updateBoolean("TransactionProduct", "isRefunded", "t",
                    txProducts.getInt(1));
            App.conn.commit();
        }
